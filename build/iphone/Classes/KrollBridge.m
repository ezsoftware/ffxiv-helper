...
NSString * FFXIVAssistant$ModuleRequireFormat = @"(function(exports){"
...
@implementation FFXIVAssistantObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ffxivassistant);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create FFXIVAssistant global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "FFXIVAssistant" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ffxivassistant = [[FFXIVAssistantObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ffxivassistant];
	
	NSString *_ffxivassistantNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ffxivassistantNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ffxivassistant valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:FFXIVAssistant$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ffxivassistant.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.ffxivassistant.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
